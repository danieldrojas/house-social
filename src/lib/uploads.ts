...
export async function saveImageFile(file: File): Promise<string> {
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const blob = await put(`houses/${filename}`, buffer, {
      access: "public",
      contentType,
  ...
  // On Vercel without Blob: store as data URL (works for friends beta after client compression)
  if (process.env.VERCEL) {
    // Cap so we don't blow Postgres row size
    if (buffer.length > 1.5 * 1024 * 1024) {
      throw new Error(
        "Photo is still too large after upload. Try a smaller image.",
      );
    }
